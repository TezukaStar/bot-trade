...
def get_last_run_time():
    """
    อ่านเวลารันล่าสุดจาก last_run.txt (ทั้ง GitHub และ local)

    Returns:
        str: เวลารันล่าสุด หรือ None ถ้าไม่มีข้อมูล
    """
    # ลอง 1: ดึงจาก GitHub
    try:
        github_url = "https://raw.githubusercontent.com/TezukaStar/bot-trade/main/last_run.txt"
        import urllib.request
        response = urllib.request.urlopen(github_url)
        last_run_str = response.read().decode('utf-8').strip()
        if last_run_str:
            ...
            if minutes_ago < 60:
                return f"{minutes_ago} นาทีที่แล้ว"
            else:
                hours_ago = int(minutes_ago / 60)
                return f"{hours_ago} ชั่วโมงที่แล้ว"
    except:
        pass

    # ลอง 2: อ่านจาก local file
...
# V1.4: แสดงคู่เงินที่เปิดใช้งานและเวลารันล่าสุด
...
with col_right:
    ...
    if last_run:
        st.info(f"**⏰ รันล่าสุด:** {last_run}")
    else:
        st.info(f"**⏰ รันล่าสุด:** ไม่มีข้อมูล")
...
PYTHONPATH=.. python3 paper_trading_simulator.py \\
  ../data/EURUSD_1m_30d.csv ../config.json""", language="bash")
        ...
        **หมายเหตุ:**
        ...
        st.markdown("""
        **ช่วงเวลาที่ Bot รันอัตโนมัติ (GitHub Actions):**
        - 🕐 **12:00-13:59 UTC** (19:00-20:59 ICT) - PUT เท่านั้น (Win Rate 73%)
          - รันทุก 30 นาที: 12:00, 12:30, 13:00, 13:30
        - 🕔 **18:00-18:59 UTC** (01:00-01:59 ICT) - CALL เท่านั้น (Win Rate 60%)
          - รันทุก 30 นาที: 18:00, 18:30

        **Bot จะเทรดเมื่อ:**
        1. GitHub Actions รันตามตารางเวลา (6 รอบ/วัน)
        2. มีสัญญาณจากกลยุทธ์ V1.4 (ตรวจทุก 30 วินาที เป็นเวลา 11 นาที)
        3. อินดิเคเตอร์ผ่านเกณฑ์ (ADX > 8, MACD, EMA20, Slope)
        4. ตรงช่วง session ที่กำหนด (PUT/CALL)

        💡 **Tip:** Dashboard อ่านข้อมูลจาก GitHub - กดปุ่ม "🔄 รีเฟรช" เพื่ออัพเดท
        """)
...
if 'selected_trade_id' in st.session_state:
    ...
    if selected_trade is None:
        ...
        if candles_loaded and len(candles) > 0:
                ...
                if 12 <= hour <= 13:
                    st.info("⏰ **ช่วงเวลา:** 12:00-13:59 UTC (19:00-20:59 ICT) - PUT session premium (Win Rate 73%)")
                elif hour == 18:
                    st.info("⏰ **ช่วงเวลา:** 18:00-18:59 UTC (01:00-01:59 ICT) - CALL session (Win Rate 60%)")
                elif 14 <= hour <= 17:
                    st.warning("⚠️ **ช่วงเวลา:** 14:00-17:59 UTC - นอกช่วง premium (ไม่แนะนำ)")
                else:
                    st.warning("⚠️ **ช่วงเวลา:** นอกช่วง schedule ที่กำหนด")
